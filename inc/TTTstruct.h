...
typedef struct _LocalVideoConfig {
    ...
    void *viewHwnd;
    int32_t enable_hwaccel;
    int32_t encode_codec;
	int32_t enable_mirror;
...
typedef struct {
    int32_t enable_video;	//是否播放视频
    int32_t enable_audio;	//是否播放音频
    int32_t enable_hwaccel; //是否使用硬件解码（qsv/vaapi）
    int32_t encode_codec;	//是否
...
typedef struct _RemoteVideoConfig  {
    ...
    void *hwnd;
    VIDEO_SCALE_TYPE renderScaleType;
    int32_t enable_hwaccel; // hardware accel
    int32_t decode_codec;
} RemoteVideoConfig;
